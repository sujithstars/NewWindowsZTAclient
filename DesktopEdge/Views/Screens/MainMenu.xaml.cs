...
namespace ZitiDesktopEdge {
	...
	public partial class MainMenu : UserControl {
		...
		public MainMenu() {
			InitializeComponent();
			Application.Current.MainWindow.Title = "Shield Endpoint";
        ...
        private void ShowIntrusion(object sender, MouseButtonEventArgs e)
        {
			SetShieldStatus();
            menuState = "Intrusion";
            UpdateState();
        ...
        async private void SetShieldStatus()
        {
            ...
            try
            {
                ...
                logger.Info("Downloading content of shield DNS...");
                jsonString = client.DownloadString("http://127.0.0.1:8001/shield?status");
                obj = JObject.Parse(jsonString);
                bool disabled = true;
                if (((string)obj["status"]).Equals("active"))
                {
                    disabled = false;
                }
                this.ShieldOn.IsSelected = !disabled;
                this.ShieldOff.IsSelected = disabled;
            }
            catch (Exception ex)
            {
                logger.Error(ex);
        ...
        async private void SetShieldStatusAction(object sender, MouseButtonEventArgs e)
        {
            try
            {
                if (sender == ShieldOn)
                {
                    WebClient client = new WebClient();
                    string jsonString = client.DownloadString("http://127.0.0.1:8001/shield?activate");
                }
                if (sender == ShieldOff)
                {
                    WebClient client = new WebClient();
                    string jsonString = client.DownloadString("http://127.0.0.1:8001/shield?deactivate");
                }
                SetShieldStatus();
            }
            catch (Exception ex)
            {
                MessageBox.Show("Error while changing shield status.\nIs the shield DNS installed?");
				logger.Error("Error while changing shield status.\n" +  ex.ToString());
                return;
            }
        }
        ...
        {
            Process.Start(new ProcessStartInfo("http://127.0.0.1:8001/") { UseShellExecute = true });
        }
		...
		private void UpdateState() {
            ...
            IntrusionItems.Visibility = Visibility.Collapsed;
			...
			} else if (menuState == "Intrusion") {
                MenuTitle.Content = "Intrusion Shield";
                IntrusionItems.Visibility = Visibility.Visible;
                BackArrow.Visibility = Visibility.Visible;
		...
		private void ShowPrivacy(object sender, MouseButtonEventArgs e) {
			Process.Start(new ProcessStartInfo("https://intrusion.com/privacy-policy/") { UseShellExecute = true });
		}
		private void ShowTerms(object sender, MouseButtonEventArgs e) {
			Process.Start(new ProcessStartInfo("https://developers.intrusion.com/terms") { UseShellExecute = true });
		}
		...
		private void ShowSupport(object sender, MouseButtonEventArgs e) {
			Process.Start(new ProcessStartInfo("https://intrusion.com/support/") { UseShellExecute = true });
		}
		...
		async private void UpdateConfig() {
			...
			try {
				...
				if (r.Code != 0) {
					...
					this.OnShowBlurb?.Invoke("Config saved - Please restart Shield Endpoint\nfor the changes to take effect");
					this.CloseEdit();
				}
        ...
        private void IdentitiesButton_Loaded()
        {
